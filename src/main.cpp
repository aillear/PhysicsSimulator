...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow("Advanced Grid System", 
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                ...
                case SDL_MOUSEMOTION:
                    if (isDragging) {
                        viewport.x = viewportStart.x + (event.motion.x - dragStart.x);
                        viewport.y = viewportStart.y + (event.motion.y - dragStart.y);
                    }
                    break;
        ...
        SDL_RenderDrawLine(renderer, 0, viewport.y, WINDOW_WIDTH, viewport.y); // X轴
        SDL_RenderDrawLine(renderer, viewport.x, 0, viewport.x, WINDOW_HEIGHT); // Y轴
